...
void visit_type_ChardevMemory(Visitor *m, ChardevMemory ** obj, const char *name, Error **errp)
{
    if (!error_is_set(errp)) {
        Error *err = NULL;
        visit_start_struct(m, (void **)obj, "ChardevMemory", name, sizeof(ChardevMemory), &err);
        if (!err) {
            if (!obj || *obj) {
...
void visit_type_ChardevMemoryList(Visitor *m, ChardevMemoryList ** obj, const char *name, Error **errp)
{
    GenericList *i, **prev = (GenericList **)obj;
    Error *err = NULL;
    ...
    if (!error_is_set(errp)) {
        ...
        if (!err) {
            for (; (i = visit_next_list(m, prev, &err)) != NULL; prev = &i) {
                ChardevMemoryList *native_i = (ChardevMemoryList *)i;
                visit_type_ChardevMemory(m, &native_i->value, NULL, &err);
            }
            error_propagate(errp, err);
            err = NULL;
            ...
            if (obj && *obj) {
                ...
                if (!err) {
                    ...
                    case CHARDEV_BACKEND_KIND_MEMORY:
                        visit_type_ChardevMemory(m, &(*obj)->memory, "data", &err);
                        break;
                    default:
                        abort();
                    }
